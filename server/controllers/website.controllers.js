...
import extractHtml from "../utils/extractHtml.js";
...
OUTPUT FORMAT
--------------------------------------------------
RETURN THE EXACT AND COMPLETE HTML DOCUMENT STARTING WITH <!DOCTYPE html> AND ENDING WITH </html>.
`;
...
export const generateWebsite = async (req, res) => {
    try {
        ...
        for (let i = 0; i < 2 && !extractedCode; i++) {
            raw = await generateResponse(finalPrompt)
            extractedCode = extractHtml(raw)
        ...
        if (!extractedCode) {
            console.log("ai returned invalid HTML", raw)
            return res.status(400).json({ message: "Invalid AI HTML" })
        }

        const website = await Website.create({
            user: user._id,
            title: prompt.slice(0, 60),
            latestCode: extractedCode,
            conversation: [
                {
                    role: "user",
                    content: prompt
...
`
        ...
        website.latestCode = extractedCode
